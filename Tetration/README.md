...
Cross validation: in my ["Calculators" repo](https://github.com/TediusTimmy/Calculators), I modified "AC6S" to use an arbitrary-precision integer for the exponent of the floating point number. The AC6S code uses a wrapper over GMP to implement arbitrary-precision integers, and builds those into a decimal arbitrary-precision floating-point number type. So, the result was a [doubly-arbitrary-precision floating point](https://github.com/TediusTimmy/CiteMyself/tree/trunk/DAPFP) type. That was nice, but there was a problem. As I computed `2^^6`, changing the precision changed all of the digits of the significand. Then, I realized that `exp()` has a loop that iterates 19700 times to reach this value. So, I need at least that many digits of precision (preferably more) to properly compute the result. This involved changing `log()` to not use hardcoded constants, and instead compute `ln(1.25) + 6 * ln(sqrt(2))` when it wants `ln(10)`. Due to the repeated raising of the result to the tenth power in `exp()`, the new computation has the same precision requirements (which amazes me, because that loop has four multiplies in it). The results match to 19 digits in the significand.
